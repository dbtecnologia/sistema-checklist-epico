...
export default function ClientsPage() {
  ...
  const [form, setForm] = useState({ legal_name: "", trade_name: "", tax_id: "", email: "", password: "" });
  ...
  async function createClient(event: FormEvent) {
    ...
    const response = await fetch("/api/admin/clientes", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify(form) });
    const result = await response.json();
    if (!response.ok) setMessage(result.error || "Não foi possível criar o cliente.");
    else { setForm({ legal_name: "", trade_name: "", tax_id: "", email: "", password: "" }); setShowForm(false); setMessage("Cliente e usuário criados com sucesso."); await load(); }
  }
  ...
  return <main style={{minHeight:"100vh",background:"#f6f8fb",padding:"34px 46px",color:"#182230",fontFamily:"Inter,system-ui,sans-serif"}}>
    <div style={{maxWidth:1100,margin:"0 auto"}}>
      ...
      {showForm && <form onSubmit={createClient} style={{background:"#fff",border:"1px solid #e7ebf0",borderRadius:12,padding:24,marginTop:22,display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        {([["legal_name","Razão social",true],["trade_name","Nome fantasia",false],["tax_id","CNPJ",false],["email","E-mail de acesso",true],["password","Senha inicial",true]] as const).map(([key,label,required])=><label key={key} style={{display:"grid",gap:6,fontSize:12,fontWeight:650}}>{label}<input required={required} type={key==="password"?"password":key==="email"?"email":"text"} minLength={key==="password"?8:undefined} value={form[key]} onChange={e=>setForm({...form,[key]:e.target.value})} style={{padding:11,border:"1px solid #dfe5ec",borderRadius:8}} /></label>)}
        <div style={{gridColumn:"1/-1",display:"flex",gap:10}}><button type="submit" style={{background:"#5667ed",border:0,color:"#fff",padding:"10px 15px",borderRadius:8,fontWeight:700}}>Criar cliente e acesso</button><button type="button" onClick={()=>setShowForm(false)} style={{background:"#eef1f5",border:0,padding:"10px 15px",borderRadius:8}}>Cancelar</button></div>
      </form>}
      {message && <p role="alert" style={{color:message.includes("sucesso")?"#218456":"#c54848",fontSize:13,marginTop:18}}>{message}</p>}
